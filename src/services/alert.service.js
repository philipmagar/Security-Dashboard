...
const  createAlert = ({ type, severity = 'medium', source, message, details = {} }) => {
    const alert = {
...
const createAlert = ({ type, severity = 'medium', source, message, details = {} }) => {
    ...
    // Keep store bounded to last 1000 alerts to prevent unbounded growth
    if (alerts.length > 1000) {
        alerts.splice(0, alerts.length - 1000);
    }
...
const getAlerts = ({ severity, type, acknowledged, limit = 50, page = 1 } = {}) => {
    let filtered = [...alerts];
