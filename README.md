...
```text
mini-siem/
├── backend-node/  # Node.js & Express API, security logging, and alerting engine
...
- **React Router Navigation:** Dedicated views for the main Dashboard and Security Logs, accessible via a persistent sidebar.
...
- **Logs Explorer:** Search, filter, and view system events in a styled, responsive data table.
...
Navigate to the `backend-node` directory, install dependencies, and start the development server.
...
*(For detailed API routing and configurations, see the internal codebase in the `/backend-node` directory).*
